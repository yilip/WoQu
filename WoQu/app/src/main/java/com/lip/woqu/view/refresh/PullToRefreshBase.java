...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout {
	/** 标识是否在onCompleteRefresh方法中调用了smoothScrool */
	...
	public final boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_UP: {
			if (isBeingDragged&&state != REFRESHING) {
				...
				if (state == RELEASE_TO_REFRESH && null != onRefreshListener) {
					...
					case MODE_PULL_DOWN_TO_REFRESH:
						onRefreshListener.onRefreshByPullDown();
						break;
					case MODE_PULL_UP_TO_REFRESH:
						onRefreshListener.onRefreshByPullUp();
						break;
    ...
    public boolean showHeaderLoadingLayout() {
        return mode == MODE_PULL_DOWN_TO_REFRESH || mode == MODE_BOTH;
    }
    ...
    public boolean showFooterLoadingLayout() {
        return mode == MODE_PULL_UP_TO_REFRESH || mode == MODE_BOTH;
    }
	...
	private void init(Context context, AttributeSet attrs) {
		...
		String pullLabel_down = context.getString(R.string.pull_to_refresh_pull_label_down);
		String pullLabel_up = "待定^_^";
		String refreshingLabel = context.getString(R.string.pull_to_refresh_refreshing_label);
		String releasedown = context.getString(R.string.pull_to_refresh_release_down);
		String releaseup = "待定^_^";

		// Add Loading Views
		if (mode == MODE_PULL_DOWN_TO_REFRESH || mode == MODE_BOTH) {
			headerLayout = new LoadingLayout(context, MODE_PULL_DOWN_TO_REFRESH, releasedown, pullLabel_down,
					refreshingLabel);
		...
		if (mode == MODE_PULL_UP_TO_REFRESH || mode == MODE_BOTH) {
			footerLayout = new LoadingLayout(context, MODE_PULL_UP_TO_REFRESH, releaseup, pullLabel_up, refreshingLabel);
    ...
    protected void onScrollChanged(int l, int t, int oldl, int oldt) {
        ...
        if (t == 0) {
            if (null != headerLayout)
                headerLayout.stopAnim();
            if (null != footerLayout)
                footerLayout.stopAnim();
        }
    }
