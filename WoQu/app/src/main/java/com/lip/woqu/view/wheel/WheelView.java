...
public class WheelView extends View {
	...
	private static final int VALUE_TEXT_COLOR = 0xFF00a1f2;

	/** Items text color */
	private static final int ITEMS_TEXT_COLOR = 0x44000000;
	...
	private final int ADDITIONAL_ITEM_HEIGHT = UtilsManager.dip2px(
            getContext(), 28);

	/** Text size */
	public int TEXT_SIZE = UtilsManager.dip2px(getContext(), 20);
	
	public int AMPM_TEXT_SIZE = UtilsManager.dip2px(getContext(), 20);
...
//	private Paint leftPaint;
	...
	private void initResourcesIfNecessary() {
		...
		if (valuePaint == null) {
			...
			valuePaint.setTextSize(UtilsManager.dip2px(getContext(), 22));
...
//		setBackgroundResource(R.drawable.wheel_bg2);
	...
	private int calculateLayoutWidth(int widthSize, int mode) {
		...
		if (widthSize==0) {
			WindowManager wm = (WindowManager) ApplicationManager.ctx.getSystemService(Context.WINDOW_SERVICE);
		...
		if (maxLength > 0) {
			float textWidth = 0;
			if (adapter!=null&&TextUtils.isDigitsOnly(adapter.getItem(0))) {
				textWidth= FloatMath.ceil(Layout.getDesiredWidth("0",
					itemsPaint));
			}else {
				textWidth= FloatMath.ceil(Layout.getDesiredWidth("00",
						itemsPaint));
			}
		...
		if(leftLabel!=null&&leftLabel.length()>0){
			leftLabelWidth=(int) FloatMath.ceil(Layout.getDesiredWidth(leftLabel,
					valuePaint));
		}
...
//			width = itemsWidth + labelWidth + 2 * PADDING;
		...
		if (recalculate) {
			...
			if(isItemsCenter){
				...
				UtilsManager.println(leftLabel+" , start ---- left:"+leftLabelWidth+" items:"+itemsWidth);
				newWidthItems = (double) itemsWidth * pureWidth/ (itemsWidth + leftLabelWidth);
				itemsWidth = (int) newWidthItems;
				leftLabelWidth = pureWidth - itemsWidth;
				UtilsManager.println(leftLabel+" , left:"+leftLabelWidth+" items:"+itemsWidth);
...
//			valuePaint.setTextSize(UtilsManager.dip2px(getContext(), 15));
			if (isSmallText) {
	...
	private void drawValue(Canvas canvas) {
		...
		ampmPaint.setColor(VALUE_TEXT_COLOR);
	...
	public boolean onTouchEvent(MotionEvent event) {
		MyScrollerView.isWeelViewFocused = true;
		WheelAdapter adapter = getAdapter();
		if (adapter == null) {
			return true;
		}
